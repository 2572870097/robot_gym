...
from legged_gym.envs.go2.go2_constraint_him import Go2ConstraintHimRoughCfg
...
from legged_gym.utils.utils import random_quat

class Go2ConstraintRobot(BaseTask):
    cfg: Go2ConstraintHimRoughCfg

    def __init__(self, cfg: Go2ConstraintHimRoughCfg, sim_params, physics_engine, sim_device, headless):
    ...
    def _init_buffers(self):
        ...
        actor_root_state = self.gym.acquire_actor_root_state_tensor(self.sim)  # 获取机器人根状态
        dof_state_tensor = self.gym.acquire_dof_state_tensor(self.sim)  # 获取机器人关节状态
        net_contact_forces = self.gym.acquire_net_contact_force_tensor(self.sim)  # 获取机器人接触力
        force_sensor_tensor = self.gym.acquire_force_sensor_tensor(self.sim)  # 获取机器人力传感器 new
        rigid_body_state_tensor = self.gym.acquire_rigid_body_state_tensor(self.sim)  # 获取机器人刚体状态

        self.gym.refresh_dof_state_tensor(self.sim)  # 刷新机器人关节状态
        self.gym.refresh_actor_root_state_tensor(self.sim)  # 刷新机器人根状态
        self.gym.refresh_net_contact_force_tensor(self.sim)  # 刷新机器人接触力
        self.gym.refresh_rigid_body_state_tensor(self.sim)  # 刷新机器人刚体状态
        self.gym.refresh_force_sensor_tensor(self.sim)  # 刷新机器人力传感器

        # create some wrapper tensors for different slices
        self.root_states = gymtorch.wrap_tensor(actor_root_state)  # 机器人根状态
        self.rigid_body_states = gymtorch.wrap_tensor(rigid_body_state_tensor).view(self.num_envs, -1, 13)  # 机器人刚体状态
        self.dof_state = gymtorch.wrap_tensor(dof_state_tensor)  # 机器人关节状态
        self.dof_pos = self.dof_state.view(self.num_envs, self.num_dof, 2)[..., 0]  # 机器人关节位置
        self.dof_vel = self.dof_state.view(self.num_envs, self.num_dof, 2)[..., 1]  # 机器人关节速度
        self.base_quat = self.root_states[:, 3:7]  # 机器人基座四元数

        self.feet_pos = self.rigid_body_states[:, self.feet_indices, 0:3]  # 机器人脚底位置
        self.feet_vel = self.rigid_body_states[:, self.feet_indices, 7:10] # 机器人脚底速度
        # 机器人脚底力传感器 new
        if force_sensor_tensor is not None:
            ...
            print("[Warning] acquire_force_sensor_tensor returned None. Using zeros placeholder. Make sure feet force sensors are created.")
            self.force_sensor_tensor = torch.zeros(self.num_envs, 4, 6, dtype=torch.float, device=self.device)
        self.contact_forces = gymtorch.wrap_tensor(net_contact_forces).view(self.num_envs, -1, 3) # shape: num_envs, num_bodies, xyz axis  # 机器人接触力
        ...
        str_rng = self.cfg.domain_rand.motor_strength_range # 电机力矩强度范围
        kp_str_rng = self.cfg.domain_rand.kp_range          # PD控制器P参数范围
        kd_str_rng = self.cfg.domain_rand.kd_range          # PD控制器D参数范围

        self.motor_strength = (str_rng[1] - str_rng[0]) * torch.rand(self.num_envs, self.num_actions, dtype=torch.float, device=self.device, requires_grad=False) + str_rng[0]   # 电机强度随机化
        self.kp_factor = (kp_str_rng[1] - kp_str_rng[0]) * torch.rand(self.num_envs, self.num_actions, dtype=torch.float, device=self.device, requires_grad=False) + kp_str_rng[0] # PD控制器参数随机化
        ...
        self.disturbance = torch.zeros(self.num_envs, self.num_bodies, 3, dtype=torch.float, device=self.device, requires_grad=False)  # 扰动向量初始化
    ...
    def _create_envs(self):
        ...
        # Attach force sensors to all feet bodies detected from the asset
        for s in feet_names:
            ...
            if feet_idx >= 0:
                ...
                print(f"[Warning] Foot body '{s}' not found in asset, skip attaching force sensor.")
    ...
    def _reward_stand_nice(self): # 站立奖励
    ...
    def _reward_foot_slide(self): # 脚底滑动约束
        cur_footvel_translated = self.feet_vel - self.root_states[:, 7:10].unsqueeze(1)
        footvel_in_body_frame = torch.zeros(self.num_envs, len(self.feet_indices), 3, device=self.device)
        for i in range(len(self.feet_indices)):
            footvel_in_body_frame[:, i, :] = quat_rotate_inverse(self.base_quat, cur_footvel_translated[:, i, :])
    ...
    def _reward_foot_clearance_hippos(self): # 脚底高度约束
        cur_footpos_translated = self.feet_pos - self.root_states[:, 0:3].unsqueeze(1)
        footpos_in_body_frame = torch.zeros(self.num_envs, len(self.feet_indices), 3, device=self.device)
        cur_footvel_translated = self.feet_vel - self.root_states[:, 7:10].unsqueeze(1)
        footvel_in_body_frame = torch.zeros(self.num_envs, len(self.feet_indices), 3, device=self.device)
    ...
    def _reward_foot_regular(self): # 脚底高度约束
        cur_footpos_translated = self.feet_pos - self.root_states[:, 0:3].unsqueeze(1)
        footpos_in_body_frame = torch.zeros(self.num_envs, len(self.feet_indices), 3, device=self.device)
    ...
    def _reward_hip_pos(self): # 髋关节位置约束
    ...
    def _reward_foot_swing_clearance(self): # 脚底摆动高度约束
    ...
    def _reward_foot_width_equlity(self): # 脚底宽度约束
        cur_footpos_translated = self.feet_pos - self.root_states[:, 0:3].unsqueeze(1)
        footpos_in_body_frame = torch.zeros(self.num_envs, len(self.feet_indices), 3, device=self.device)
        for i in range(len(self.feet_indices)):
            footpos_in_body_frame[:, i, :] = quat_rotate_inverse(self.base_quat, cur_footpos_translated[:, i, :])
    ...
    def _reward_foot_dia_enforce(self): # 脚底对角线长度约束
        cur_footpos_translated = self.feet_pos - self.root_states[:, 0:3].unsqueeze(1)
        footpos_in_body_frame = torch.zeros(self.num_envs, len(self.feet_indices), 3, device=self.device)
        for i in range(len(self.feet_indices)):
            footpos_in_body_frame[:, i, :] = quat_rotate_inverse(self.base_quat, cur_footpos_translated[:, i, :])
    ...
    def _reward_foot_width_cons(self): # 脚底宽度约束
        cur_footpos_translated = self.feet_pos - self.root_states[:, 0:3].unsqueeze(1)
        footpos_in_body_frame = torch.zeros(self.num_envs, len(self.feet_indices), 3, device=self.device)
        for i in range(len(self.feet_indices)):
            footpos_in_body_frame[:, i, :] = quat_rotate_inverse(self.base_quat, cur_footpos_translated[:, i, :])
    ...
    def _reward_foot_mirror(self): # 脚底镜像约束
        diff1 = torch.sum(torch.square(self.dof_pos[:,[0,1,2]] - self.dof_pos[:,[9,10,11]]),dim=-1)
        diff2 = torch.sum(torch.square(self.dof_pos[:,[3,4,5]] - self.dof_pos[:,[6,7,8]]),dim=-1)
        return 0.5*(diff1 + diff2)
    
    def _reward_trot_contact(self):  #  trot 接触约束
        contact_filt = 1.*self.contact_filt
        ...
        return pattern_match_flag*(torch.norm(self.commands[:, :3], dim=1) > 0.1)  # 修改2->3，添加了z轴方向
