...
if __name__ == "__main__":
    ...
    with open(f"{LEGGED_GYM_ROOT_DIR}/deploy/deploy_mujoco_lab/configs/{config_file}", "r") as f:
    ...
    # load policy
    policy = torch.jit.load(policy_path, map_location=torch.device('cpu'))
    policy.eval()  # 设置为评估模式
            ...
            if counter % control_decimation == 0:
                ...
                with torch.no_grad():
                    ...
                    if isinstance(action, tuple):
                        action = action[0]  # 如果模型输出是元组，取第一个元素
                    action = action.cpu().numpy().squeeze()
                # transform action to target_dof_pos
                print(f"action: {action}")
                target_dof_pos = action * np.array(action_scale) + default_angles
